"""
management/commands/import_2024_pdf.py

Imports 2024 historical event data from a PDF (or hardcoded dataset) into
HistoricalEventReference records, then optionally writes Markdown reports.

Usage:
    python manage.py import_2024_pdf
    python manage.py import_2024_pdf --pdf /path/to/2024.pdf
    python manage.py import_2024_pdf --dry-run
    python manage.py import_2024_pdf --generate-reports
"""
import os
...
class Command(BaseCommand):
    help = "Import 2024 historical event data and match against CRM events."
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            dest="pdf_path",
            default=None,
            help="Absolute path to the 2024 PDF file. If omitted the hardcoded dataset is used.",
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("[import_2024_pdf] Starting 2024 historical event import.")
        if dry_run:
            self.stdout.write("[import_2024_pdf] DRY-RUN mode -- no DB writes.")

        service = Historical2024ImportService(pdf_path=pdf_path, dry_run=dry_run)
        summary = service.run()

        self.stdout.write("")
        self.stdout.write("=== 2024 Import Summary ===")
        ...
        self.stdout.write("")
        self.stdout.write("[import_2024_pdf] Done.")

    # ------------------------------------------------------------------
    def _write_error_report(self, summary):
        errors = summary.get("errors", [])
        lines = [
            "# HISTORICAL 2024 IMPORT ERRORS",
            "",
        ...
        out_path = REPORT_DIR / "HISTORICAL_2024_IMPORT_ERRORS.md"
        _write_report(out_path, lines)
        self.stdout.write(f"[import_2024_pdf] Error report written to: {out_path}")

    def _write_success_report(self, summary):
        successes = summary.get("successes", [])
        lines = [
            "# HISTORICAL 2024 IMPORT SUCCESSES",
            "",
            f"Total rows processed : {summary['total']}",
            f"Verified             : {summary['verified']}",
            f"Dry-run              : {summary['dry_run']}",
        ...
        out_path = REPORT_DIR / "HISTORICAL_2024_IMPORT_SUCCESS.md"
        _write_report(out_path, lines)
        self.stdout.write(f"[import_2024_pdf] Success report written to: {out_path}")
