"""
management/commands/create_missing_2025_events.py

Dynamically reads all HistoricalEventReference objects with event_year=2025
and verification_status='unmatched', creates a CRM Event for each unique
unmatched code, then wipes and re-runs the 2025 import service so those
records can now be matched.

Usage:
    python manage.py create_missing_2025_events
    python manage.py create_missing_2025_events --dry-run
"""
import datetime
...
def _derive_event_fields(ref: HistoricalEventReference) -> dict:
    ...
    event_date, end_date = _parse_date_range(date_range, 2025)
    ...
    # --- name ---
    name = f"{ref.normalized_event_code} 2025"
...
class Command(BaseCommand):
    help = (
        "Dynamically create CRM Event records for all 2025 unmatched historical "
        "references, then wipe unmatched rows and re-run the 2025 import."
    )
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            self.stdout.write("[create_missing_2025_events] DRY-RUN mode -- no DB writes.")

        # ------------------------------------------------------------------
        # 1. Find all unique unmatched 2025 codes
        # ------------------------------------------------------------------
        self.stdout.write("")
        self.stdout.write("=== Step 1: Reading unmatched 2025 HistoricalEventReference rows ===")

        unmatched_refs = (
            HistoricalEventReference.objects
            .filter(event_year=2025, verification_status=HistoricalEventReference.VerificationStatus.UNMATCHED)
            .order_by("normalized_event_code", "event_month")
        )

        total_unmatched = unmatched_refs.count()
        self.stdout.write(f"  Found {total_unmatched} unmatched 2025 reference(s).")

        if total_unmatched == 0:
            self.stdout.write(
                "  No unmatched 2025 references found.  "
                "Run 'import_2025_pdf' first to populate the table."
            )
        ...
        # ------------------------------------------------------------------
        self.stdout.write("")
        self.stdout.write("=== Step 2: Creating missing 2025 Event records ===")
        created_count = 0
        skipped_count = 0
        ...
        # ------------------------------------------------------------------
        # 3. Wipe all unmatched 2025 HistoricalEventReference rows
        # ------------------------------------------------------------------
        self.stdout.write("")
        self.stdout.write("=== Step 3: Wiping unmatched 2025 HistoricalEventReference rows ===")
        ...
        if dry_run:
            self.stdout.write(
                f"  [DRY-RUN] Would delete {wipe_count} unmatched 2025 reference(s)."
            )
        else:
            deleted, _ = wipe_qs.delete()
            self.stdout.write(f"  Deleted {deleted} unmatched 2025 reference(s).")

        # ------------------------------------------------------------------
        # 4. Re-run 2025 import service
        # ------------------------------------------------------------------
        self.stdout.write("")
        self.stdout.write("=== Step 4: Re-running 2025 historical import service ===")

        service = Historical2025ImportService(dry_run=dry_run)
        summary = service.run()

        self.stdout.write("")
        self.stdout.write("--- 2025 Re-import Summary ---")
        ...
        self.stdout.write("")
        self.stdout.write("[create_missing_2025_events] Done.")
