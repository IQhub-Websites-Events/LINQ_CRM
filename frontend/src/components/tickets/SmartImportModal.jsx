...
import { APP_FIELDS, ALIAS_MAP } from "./import/fieldDefinitions";
...
function fmtDateDisplay(d) {
  // Return ISO YYYY-MM-DD — parseable by the backend _parse_date and unambiguous.
...
function detectExcelDateCols(columns, rows) {
  ...
  for (const col of columns) {
    for (const row of sample) {
      const v = row[col];
      // D27: narrower range matching bookings/import/dateFormatter.js isExcelSerial (32874–73050)
      if (typeof v === "number" && v >= 32874 && v <= 73050) { dateCols.add(col); break; }
    }
  }
  return dateCols;
}
...
async function parseXLSXFile(file) {
  ...
  // D27: cellDates:true makes SheetJS convert date-formatted cells to JS Date objects.
  // This is the primary fix for the "date column showing raw serial" user bug.
  ...
  // dateCols: fallback for cells that still arrive as numbers (poorly-formatted cells
  // without a date number-format applied, so cellDates:true didn't convert them).
  ...
  const data = rawRows.map(row => {
    ...
    for (const col of columns) {
      ...
      if (raw instanceof Date) {
        // cellDates:true gave us a real Date — format as ISO for backend
        out[col] = fmtDateDisplay(raw);
      } else if (dateCols.has(col) && typeof raw === "number") {
        // Fallback: cell came through as serial despite cellDates:true
        out[col] = fmtDateDisplay(excelSerialToDate(raw));
      } else {
...
function Step3Validation({ data, mapping, fileName, duplicateMode, setDuplicateMode, dryRun, setDryRun, onBack, onNext }) {
  const issues = [];

  // Error: ticket_number (the only required field) not mapped
  if (!mapping["ticket_number"]) {
    issues.push({ level: "error", message: `"Ticket Number" is required but not mapped — rows without a ticket number will be skipped` });
  } else {
    const emptyKey = data.filter(row => !(row[mapping["ticket_number"]] || "").trim()).length;
    if (emptyKey > 0) {
      issues.push({
        level: "warn",
        message: `${emptyKey.toLocaleString()} row${emptyKey !== 1 ? "s" : ""} have an empty Ticket Number — they will be skipped at import`,
      });
    }
  }
  ...
  return (
    <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{
        ...
        <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: TD, marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.04em" }}>
          Duplicate Handling
        </label>
        <select
          value={duplicateMode}
        ...
        >
          <option value="allow_all">
            Allow all rows (no deduplication) — every row creates a new ticket
          </option>
          <option value="skip_by_external_id">
            Skip duplicates by Zoho ID — recommended for migration
          </option>
          <option value="upsert_by_external_id">
            Update duplicates by Zoho ID — overwrites existing rows
          </option>
          <option value="skip_by_ticket_number">
            Skip duplicates by Ticket Number (legacy behavior)
          </option>
        </select>
        <div style={{ fontSize: 11.5, color: TF, marginTop: 8, lineHeight: 1.4 }}>
          {duplicateMode === "allow_all" && (
            "Every CSV row becomes a separate ticket. Use this when the same ticket number appears multiple times by design."
          )}
          {duplicateMode === "skip_by_external_id" && (
            "Recommended for Zoho migration. Skips rows whose Zoho ID already exists."
          )}
          {duplicateMode === "upsert_by_external_id" && (
            "Re-imports update existing tickets. Safe to re-run on the same file."
          )}
          {duplicateMode === "skip_by_ticket_number" && (
            "⚠️ Will skip rows that share a ticket number — only use if ticket numbers are truly unique in your data."
          )}
        </div>
      </div>
      ...
      <label style={{
        ...
        fontSize: 12.5, color: TD, userSelect: "none",
        background: dryRun ? "rgba(99,102,241,0.06)" : SA,
        border: `1px solid ${dryRun ? "rgba(99,102,241,0.35)" : B}`,
        borderRadius: 7, padding: "8px 12px",
      }}>
        <input
          type="checkbox"
          ...
          style={{ width: 14, height: 14, accentColor: ACC, cursor: "pointer" }}
        />
        <span>
          <strong style={{ color: dryRun ? "#6366f1" : T }}>Dry Run</strong>
          {" — validate all rows and show counts without writing to the database"}
        </span>
      </label>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 4 }}>
        ...
        <button
          ...
          style={{
            height: 34, padding: "0 20px", borderRadius: 5,
            background: hasErrors ? "var(--surface)" : dryRun ? "#6366f1" : ACC,
...
function Step4Progress({ data, mapping, duplicateMode, dryRun, fileName, onDone }) {
  const toast    = useToast();
  const started  = useRef(false);
  const [progress, setProgress] = useState({ processed: 0, total: data.length, inserted: 0, updated: 0, skipped: 0, errors: [], problemRows: [] });
  ...
  const runImport = async () => {
    ...
    let skipped          = 0;
    const allErrors      = [];
    const allProblemRows = []; // {row_index (global), ticket_number, reason/message}
    ...
    for (let i = 0; i < total; i += BATCH_SIZE) {
      ...
      try {
        const result = await ticketCentralApi.bulkImport({
        ...
        // dry_run responses use would_insert/would_update/would_skip keys
        inserted += (dryRun ? result.would_insert : result.inserted)  || 0;
        updated  += (dryRun ? result.would_update : result.updated)   || 0;
        skipped  += (dryRun ? result.would_skip   : result.skipped_count) || 0;
        ...
        if (result.skipped_rows?.length) {
          result.skipped_rows.forEach(s => {
            let reasonLabel = s.reason;
            if (s.reason === "duplicate_external_id") reasonLabel = "Duplicate Zoho ID";
            else if (s.reason === "in_file_duplicate_external_id") reasonLabel = "In-file Duplicate Zoho ID";
            else if (s.reason === "duplicate_ticket_number") reasonLabel = "Duplicate Ticket Number";
            else if (s.reason === "in_file_duplicate_ticket_number") reasonLabel = "In-file Duplicate Ticket Number";
      ...
      setProgress({ processed, total, inserted, updated, skipped, errors: allErrors, problemRows: allProblemRows });
    ...
    setDone(true);
    if (dryRun) {
      toast.success(`Dry run complete — ${(inserted + updated).toLocaleString()} would be written, no data changed`);
    } else if (allErrors.length === 0) {
      toast.success(`${(inserted + updated).toLocaleString()} tickets imported (${inserted} new, ${updated} updated)`);
    } else {
  ...
  const downloadProblemRows = () => {
    ...
    const header  = ["Row #", "Ticket Number", "Reason", ...srcCols];
    const lines   = [header.join(",")];
    problemRows.forEach(({ row_index, ticket_number, reason }) => {
      const srcRow = data[row_index] || {};
      const vals   = srcCols.map(c => {
        const v = String(srcRow[c] ?? "").replace(/"/g, '""');
        return `"${v}"`;
      });
      lines.push([row_index + 1, `"${ticket_number}"`, `"${reason}"`, ...vals].join(","));
    });
  ...
  return (
    <div style={{ padding: "28px 24px", display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {(progress.inserted > 0 || progress.updated > 0 || progress.skipped > 0 || done) && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>
          {[
            { label: dryRun ? "Would Insert" : "Inserted", value: progress.inserted.toLocaleString(),      color: "#10b981" },
            { label: dryRun ? "Would Update" : "Updated",  value: progress.updated.toLocaleString(),       color: "#6366f1" },
            { label: dryRun ? "Would Skip"   : "Skipped",  value: progress.skipped.toLocaleString(),       color: "#f59e0b" },
      ...
      {done && (
        <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 10 }}>
          ...
          <button onClick={dryRun ? () => { /* stay open so user can uncheck dry-run and re-run */ onDone(); } : onDone} style={{
            height: 36, padding: "0 28px", borderRadius: 6,
            background: dryRun ? "#6366f1" : ACC, border: "none", color: "#fff",
            fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit",
          }}>
            {dryRun ? "Dry Run Complete — Close" : "Done — View Tickets"}
          </button>
        </div>
      )}
    </div>
...
export function SmartImportModal({ open, onClose, onDone }) {
  ...
  const [duplicateMode, setDuplicateMode] = useState("allow_all");
  const [dryRun,   setDryRun]   = useState(false);

  const reset = () => { setStep(1); setFileData(null); setMapping({}); setDuplicateMode("allow_all"); setDryRun(false); };
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: TF, marginTop: 1 }}>
              Import tickets from .xlsx, .csv, or .json files
            </div>
          </div>
          <button
            onClick={handleClose}
